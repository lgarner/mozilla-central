...
let mockApp = (localId, manifestURL, certHashArray) => {
  window.DOMApplicationRegistry = {
    ...
    getManifestURLByLocalId: (_localId) => {
      if (localId !== _localId) {
        return null;
      }
    ...
    getManifestFor: (_manifestURL) => {
      return new Promise(function (resolve, reject) {
        ...
        resolve({
          secure_element_sig: SEUtils.byteArrayToHexString(certHashArray)
        });
      });
    }
  };
...
scenarios.forEach((scenario) => {
  ...
  describe("Scenario: " + scenario.name, () => {
    ...
    beforeEach((done) => {
      ...
      // Hack, for a lack of callback/promise in init()
      setTimeout(function() {
        done();
      }, 50);
    });
    ...
    it("Rules should be read to an internal representation.", (done) => {
      ...
      mockApp(localId, manifestURL, [1, 2, 3]);

      ace.isAccessAllowed(localId, "uicc", [])
      .then(() => {
        let actual = ace._rulesManagers.get("uicc").rules;
    ...
    describe("Decision making.", function() {
      for (let decisionExpectation of scenario.decisionAsserts) {
        it(decisionExpectation.name, (done) => {
          ...
          mockApp(localId, manifestURL, decisionExpectation.application);

          let applet = decisionExpectation.applet;

          ace.isAccessAllowed(localId, "uicc", applet)
          .then((decision) => {
            console.log('decision is', decision);
            expect(decision, "invalid decision").to.equal(decisionExpectation.expectedDecision);
            done();
          });
        });
      }
...
describe('Scenario: ' + refreshTag.name, function() {
  ...
  it('Reads all files at ACE init, but subsequently only up to ACMF.', function(done) {
    let assertSuccess = (ace) => {
      let actual = ace._rulesManagers.get("uicc").rules;
      expect(MockUiccConnector.mockPosition).to.equal(refreshTag.steps.length);
    ...
    setTimeout(() => {
      ...
      .then((decision) => {
        assertSuccess(ace);
        return ace.isAccessAllowed(localId, "uicc", applet);
      }).then((decision) => {
        assertSuccess(ace);
        done();
      });
