...
GPAccessDecision.prototype = {
  isAccessAllowed: function isAccessAllowed() {
    // GPD SE Access Control v1.1, 3.4.1, Table 3-2
  ...
  _decideAppAccess: function _decideAppAccess(rule) {
    ...
    //
    // DeviceAppID is the application, and the SE app id is AID:
    //
    // GPD SE AC 4.2.3 A:
    ...
    if (Array.isArray(rule.application)) {
      appMatched = this._hashMatches(rule.application);
    }
  ...
  _hashMatches: function _hashMatches(appArray) {
...
function ACEService() {
  ...
  this._rulesManagers.forEach((ruleManager, seType) => {
    ruleManager.getAccessRules().then(() => {
