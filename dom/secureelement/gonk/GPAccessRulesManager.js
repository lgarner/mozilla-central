...
GPAccessRulesManager.prototype = {
  ...
  _readACMF: function _readACMF(dodfFile) {
    ...
    let records = this._ensureIsArray(dodfFile[TAG_EXTERNALDO]);
  ...
  _readACConditions: function _readACConditions(acRulesFile) {
    ...
    let acRules = this._ensureIsArray(acRulesFile[TAG_SEQUENCE]);
    if (acRules.length === 0) {
    ...
    acRules.forEach((ruleEntry) => {
      ...
      if (condition === null) {
        ...
        if (!Array.isArray(condition[TAG_SEQUENCE]) &&
          ...
          rule.application = this._ensureIsArray(condition[TAG_SEQUENCE])
                             .map((conditionEntry) => {
