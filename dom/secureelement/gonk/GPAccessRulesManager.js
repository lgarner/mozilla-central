...
GPAccessRulesManager.prototype = {
  ...
  // FIXME: This DOMString is due to the nsIIccProvider interface.
  ...
  REFRESH_TAG_PATH: [TAG_SEQUENCE, TAG_OCTETSTRING],
  refreshTag: null,
  ...
  _readBinaryFile: function _readBinaryFile(selectResponse) {
    ...
    let fileLength = selectResponse[TAG_FCP][0x80];
  ...
  _readDODF: function _readDODF(odfFile) {
    ...
    let DODF_DF = odfFile[TAG_EF_ODF][TAG_SEQUENCE][TAG_OCTETSTRING];
  ...
  _readACMF: function _readACMF(dodfFile) {
    ...
    let records = SEUtils.ensureIsArray(dodfFile[TAG_EXTERNALDO]);

    // Look for the OID registered for GPD SE.
    let gpdRecords = records.filter((record) => {
      let oid = record[TAG_EXTERNALDO][TAG_SEQUENCE][TAG_OID];
    ...
    let ACMain_DF = gpdRecords[0][TAG_EXTERNALDO][TAG_SEQUENCE]
                                 [TAG_SEQUENCE][TAG_OCTETSTRING];
  ...
  _readACRules: function _readACRules(acMainFile) {
    ...
    let ACRules_DF = acMainFile[TAG_SEQUENCE][TAG_SEQUENCE][TAG_OCTETSTRING];
  ...
  _readACConditions: function _readACConditions(acRulesFile) {
    ...
    let acRules = SEUtils.ensureIsArray(acRulesFile[TAG_SEQUENCE]);
    if (acRules.length === 0) {
    ...
    acRules.forEach((ruleEntry) => {
      let df = ruleEntry[TAG_SEQUENCE][TAG_OCTETSTRING];
  ...
  _parseRules: function _parseRules(acRulesFile, acConditionFiles) {
    ...
    acRules.forEach((ruleEntry) => {
      ...
      let oneApplet = ruleEntry[TAG_GPD_AID];
      let allApplets = ruleEntry[TAG_GPD_ALL];

      if (oneApplet) {
        rule.applet = oneApplet[TAG_OCTETSTRING];
      } else if (allApplets) {
      ...
      } else if (condition[TAG_SEQUENCE]) {
        if (!Array.isArray(condition[TAG_SEQUENCE]) &&
            !condition[TAG_SEQUENCE][TAG_OCTETSTRING]) {
          rule.application = Ci.nsIAccessRulesManager.ALLOW_ALL;
        } else {
          rule.application = SEUtils.ensureIsArray(condition[TAG_SEQUENCE])
                             .map((conditionEntry) => {
            return conditionEntry[TAG_OCTETSTRING];
          });
        }
      } else {
