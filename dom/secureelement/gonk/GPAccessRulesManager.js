...
const FCP = 0x62; // File control parameters template
const FMD = 0x64; // File management data template
const FCI = 0x6F; // File control information template
...
GPAccessRulesManager.prototype = {
  ...
  _openChannel: function _openChannel() {
    if (this.channel !== null) {
      DEBUG && debug("_openChannel: Channel already opened, rejecting.");
  ...
  _closeChannel: function _closeChannel() {
    if (this.channel === null) {
      DEBUG && debug("_closeChannel: Channel not opened, rejecting.");
      return Promise.reject();
    }
  ...
  _exchangeAPDU: function _exchangeAPDU(bytes) {
    if (this.channel === null) {
      DEBUG && debug("Channel not opened");
      return Promise.reject();
    }
  ...
  _readBinaryFile: function _readBinaryFile(selectResponse) {
    ...
    let fileLength = selectResponse[FCP][0x80]; // 0x80: file length.
  ...
  _readDODF: function _readDODF(odfFile) {
    ...
    let DODF_DF = odfFile[0xA7][0x30][0x04]; // EF_ODF, ASN.1 sequence, DODF
  ...
  _readACMF: function _readACMF(dodfFile) {
    ...
    let gpdRecords = records.filter((record) => {
      // [External data objects][ASN.1 Seq][OID tag]
  ...
  /* Parse "Type Length Value" (TLV) from bytes. */
