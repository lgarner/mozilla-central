...
const NFC_IPC_WRITE_PERM_MSG_NAMES = [
  ...
  "NFC:SendFile",
  "NFC:RegisterEventTarget",
  "NFC:UnregisterEventTarget"
];
...
XPCOMUtils.defineLazyGetter(this, "gMessageManager", function () {
  return {
    ...
    registerEventTarget: function registerEventTarget(msg) {
      let appInfo = msg.json;
      // Sanity check on DOMEvent
      debug("XXXXXXXXXXXX Register event?. ID: [" + appInfo.event + "] XXXXXXXXXXXXXX");
      if (!this.isValidDOMEvent(appInfo.event)) {
        return;
      }
      debug("XXXXXXXXXXXX Registered event. ID: [" + appInfo.event + "] XXXXXXXXXXXXXX");
      let targets = this.eventTargetsMap;
    ...
    unregisterEventTarget: function unregisterEventTarget(msg) {
      let appInfo = msg.json;
      // Sanity check on DOMEvent
      if (!this.isValidDOMEvent(appInfo.event)) {
        return;
    ...
    isRegisteredP2PTarget: function isRegisteredP2PTarget(appId, event) {
      let targetInfo = this.eventTargetsMap[appId];
      // Check if it is a registered target for the 'event'
      return ((targetInfo != null) && ((targetInfo.event & event) !== 0));
    },

    notifyDOMEvent: function notifyDOMEvent(appId, event, message) {
      let targetInfo = this.eventTargetsMap[appId];
    ...
    isValidDOMEvent: function isValidDOMEvent(event) {
      switch(event) {
        case NFC.NFC_PEER_EVENT_READY:
        case NFC.NFC_PEER_EVENT_LOST:
        case NFC.NFC_HCI_EVENT_TRANSACTION:
          return true;
        default:
          return false;
      }
    ...
    receiveMessage: function receiveMessage(msg) {
      ...
      switch (msg.name) {
        ...
        case "NFC:RegisterEventTarget":
          this.registerEventTarget(msg);
          return null;
        case "NFC:UnregisterEventTarget":
          this.unregisterEventTarget(msg);
          return null;
        ...
        case "NFC:NotifyUserAcceptedP2P":
          ...
          this.notifyDOMEvent(msg.json.appId, NFC.NFC_PEER_EVENT_READY, null);
          return null;
...
Nfc.prototype = {
  ...
  notifyHCIEventTransaction: function notifyHCIEventTransaction(message) {
    ...
    // fire to all of them.
    let targets = gMessageManager.eventTargetsMap;

    for (let appId in targets) {
       debug("Application ID: " + appId);
      if (targets[appId].event & NFC.NFC_HCI_EVENT_TRANSACTION) {
        delete message.type;
        gMessageManager.notifyDOMEvent(appId, NFC.NFC_HCI_EVENT_TRANSACTION, message);
      }
    }
  },
  ...
  onEvent: function onEvent(event) {
    ...
    switch (message.type) {
      ...
      case "TechLostNotification":
        ...
        gMessageManager.notifyDOMEvent(gMessageManager.currentPeerAppId, NFC.NFC_PEER_EVENT_LOST, null);
