...
NfcWorker[NFC_NOTIFICATION_HCI_EVENT_TRANSACTION] = function NFC_NOTIFICATION_HCI_EVENT_TRANSACTION() {
  debug("NFC_NOTIFICATION_HCI_EVENT_TRANSACTION");
  let aidLen = Buf.readInt32();
  let aid = Buf.readUint8Array(aidLen);
  let padding;
  padding = getPaddingLen(aidLen);
  for (let i = 0; i < padding; i++) {
    Buf.readUint8();
  }

  let payloadLen = Buf.readInt32();
  let payload = Buf.readUint8Array(payloadLen);
  padding   = getPaddingLen(payloadLen);
  for (let i = 0; i < padding; i++) {
    Buf.readUint8();
  }

  let uri = "nfc://secure:0/";
  let originSE = "nothingtoseehere";

  this.sendDOMMessage({type: "HCIEventTransaction",
                       sessionId: null,
                       mimetype: "text/aid",
                       uri: "nfc://secure:0/nothingtoseehere/anAID",
                       extra: {
                         aid: aid,
                         data: payload
                       }
