...
#define COPY_OPT_FIELD(prop, defaultValue)            \
  if (aOther.prop.WasPassed()) {                      \
    ...
    if (aOther.mHCIEventTransaction.WasPassed()) {
      if (aOther.mHCIEventTransaction.mAid.WasPassed() {
        aOther.mHCIEventTransaction.mAid.Value().ComputeLengthAndData();
        mHCIEventTransaction.AppendElements(aOther.mHCIEventTransaction.mAid.Value().Data(),
                                            aOther.mHCIEventTransaction.mAid.Value().Length());
      }

      if (aOther.mHCIEventTransaction.mPayload.WasPassed() {
        aOther.mHCIEventTransaction.mPayload.Value().ComputeLengthAndData();
        mHCIEventTransaction.AppendElements(aOther.mHCIEventTransaction.mPayload.Value().Data(),
                                            aOther.mHCIEventTransaction.mPayload.Value().Length());
      }

      if (aOther.mHCIEventTransaction.mAid.WasPassed() {
        aOther.mHCIEventTransaction.mAidOrigin.Value().ComputeLengthAndData();
        mHCIEventTransaction.AppendElements(aOther.mHCIEventTransaction.mAidOrigin.Value().Data(),
                                            aOther.mHCIEventTransaction.mAidOrigin.Value().Length());
      }
    }
