...
HCIEventTransactionSystemMessageConfigurator.prototype = {
  ...
  shouldDispatch: function shouldDispatch(manifestURL, pageURL, type, message, extra) {
    ...
    let evtTransaction = message.hciEventTransaction;
    if(!evtTransaction) {
      return false;
    }
    ...
    let self = this;
    appsService.getManifestFor(manifestURL)
    .then((manifest) => {
      return self._checkAppManifest(seName, aid, manifest)
    })
    .then(() => {
  ...
  _checkAppManifest: function _checkAppManifest(seName, aid, manifest) {
    debug('manifest ' + JSON.stringify(manifest));

    let seManifest = manifest['secure_element_access'] || [];
    if (!seManifest || (seManifest.length <= 0)) {
      return Promise.reject();
    }

    let hciRules = seManifest["hci-evt-transaction-src"] || [];
